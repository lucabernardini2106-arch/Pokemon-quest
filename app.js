...
const GROUPS=[
["🧸","Riordinare insieme la cameretta",15],["🛋️","Riordinare insieme il salone",15],["🧹","Fare insieme una piccola pulizia della casa",20],["🧺","Raccogliere e sistemare insieme i giochi",15],["📚","Mettere in ordine insieme libri e materiale scolastico",15],["🍽️","Preparare insieme la tavola per un pasto",15],["🥣","Preparare insieme la colazione",20],["🍕","Aiutare insieme a preparare la cena",20],["🧩","Fare un gioco da tavolo tutti insieme",15],["🎨","Fare un disegno o lavoretto insieme",15],["🧱","Costruire qualcosa insieme con LEGO o costruzioni",20],["📖","Leggere insieme una storia",15],["🎭","Inventare e mettere in scena una piccola storia",20],["🧠","Risolvere insieme un enigma o rompicapo",20],["🔎","Fare una piccola caccia al tesoro in casa",20],["🌳","Fare una passeggiata insieme",20],["⚽","Giocare insieme all'aperto",20],["🎵","Fare insieme 15 minuti di musica o canto",15],["🧹","Sistemare insieme una zona della casa scelta dai genitori",20],["❤️","Fare qualcosa di gentile per un altro membro della famiglia",15]
];
...
function dailyQs(k){
 let all=assigned(k).filter(q=>!isRotation(q));
 const fixed=FIXED.map(f=>{let q=s.lib.find(x=>x.text===f.text);if(!q){q={id:id(),...f};s.lib.push(q)}else{q.icon=f.icon;q.xp=f.xp}return q});
 let variable=all.filter(q=>!isFixed(q));
 let seed=[...today(),...String(k.id)].reduce((a,c)=>((a*31+c.charCodeAt(0))>>>0),7);
 variable=variable.map(q=>({q,r:((seed=(seed*1664525+1013904223)>>>0)/4294967296)})).sort((a,b)=>a.r-b.r).map(x=>x.q);
 return fixed.concat(variable.slice(0,5));
}
...
function bind(){
 ...
 document.querySelectorAll("[data-q]").forEach(b=>b.onclick=async()=>{
   ...
   if(d){
    ...
    delete k.done[key];delete k.done[key+"|earned"];
    k.xp=Math.max(0,k.xp-earned);k.weekly=Math.max(0,k.weekly-earned);
    let remaining=qs.filter(x=>done(k,x)).length;
    let bonus=Number(k.dailyBonus?.[today()]||0);
    if(bonus && remaining<qs.length){
      k.xp=Math.max(0,k.xp-bonus);k.weekly=Math.max(0,k.weekly-bonus);
      delete k.dailyBonus[today()];
    }
   }
   else{
    k.done[key]=1;k.done[key+"|earned"]=base;k.xp=Math.min(3000,k.xp+base);k.weekly+=base;
    // Completing all 10 Quest gives a completion marker, not extra XP.
    let all=qs.length>0 && qs.every(x=>done(k,x));
    if(all){k.dailyBonus[today()]=1}
   }
   await save();render();
 });
}
function bindGroup(){document.querySelectorAll("[data-gq]").forEach(b=>b.onclick=async()=>{let k=s.kids.find(x=>x.id===active),q=groupQuest();if(!k||!q)return;k.groupDone=k.groupDone||{};let xp=q.xp||15;if(groupDone(k)){let earned=Number(k.groupDone["xp|"+today()]||xp);delete k.groupDone[today()];delete k.groupDone["xp|"+today()];k.xp=Math.max(0,k.xp-earned);k.weekly=Math.max(0,k.weekly-earned)}else{k.groupDone[today()]=1;k.groupDone["xp|"+today()]=xp;k.xp=Math.min(3000,k.xp+xp);k.weekly+=xp}await save();render()})}
