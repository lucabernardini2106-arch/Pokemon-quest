...
function kid(n,a){return{id:id(),name:n,age:a,xp:0,weekly:0,line:0,done:{}}}
...
function render(){
 ...
 <div class="row between"><b style="font-size:1.3rem">${p[z]}</b><span class="chip">Lv ${l}</span></div>
...
function pane(k){
 if(tab==="quests"){
  let qs=assigned(k),doneN=qs.filter(q=>done(k,q)).length;
  if(k.age<=6){
   ...
   ${qs.map(q=>{let d=done(k,q);return `<div class="quest ${d?"done":""}"><span class="icon">${q.icon}</span><span class="text">${esc(q.text)}</span><button data-speak="${esc(q.text)}">🔊</button><button data-q="${q.id}">${d?"↩️":"✅"}</button></div>`}).join("")}
   <div class="small">Ogni missione: +20 XP. Tutte completate: bonus +40 XP.</div></div>`;
  }
  return `<div class="pane active card"><div class="row between"><h2>Quest di oggi</h2><span class="chip">${doneN}/${qs.length}</span></div>
  ${qs.map(q=>{let d=done(k,q);return `<div class="quest ${d?"done":""}"><span class="icon">${q.icon}</span><span class="text">${esc(q.text)}</span><button data-q="${q.id}">${d?"↩️ Annulla":"✅ Fatto"}</button></div>`}).join("")}
  <div class="small">+20 XP per Quest. Completando tutte le Quest: bonus +40 XP.</div></div>`;
 }
...
function bind(){
 ...
 document.querySelectorAll("[data-q]").forEach(b=>b.onclick=async()=>{
   ...
   if(d){delete k.done[key];k.xp=Math.max(0,k.xp-20);k.weekly=Math.max(0,k.weekly-20)}
   else{k.done[key]=1;k.xp=Math.min(3000,k.xp+20);k.weekly+=20;if(assigned(k).every(x=>done(k,x))){k.xp=Math.min(3000,k.xp+40);k.weekly+=40}}
   await save();render();
 });
}
...
function adminPane(t){let b=$("#ab");if(t==="kids")b.innerHTML=`<div class="card"><button id="add">＋ Aggiungi bambino</button></div>${s.kids.map(k=>`<div class="list"><label>Nome</label><input data-n="${k.id}" value="${esc(k.name)}"><label>Età (fino a 6 = modalità semplificata)</label><input data-age="${k.id}" type="number" min="3" max="14" value="${k.age}"><button data-save="${k.id}">Salva</button> <button data-del="${k.id}" style="background:#d63031">Elimina</button></div>`).join("")}`;else if(t==="quests")b.innerHTML=`<div class="card"><button id="newq">＋ Nuova Quest</button></div>${s.lib.map(q=>`<div class="list"><b>${q.icon} ${esc(q.text)}</b><div class="small">Assegnata a: ${s.kids.filter(k=>(s.assign[k.id]||[]).includes(q.id)).map(k=>esc(k.name)).join(", ")||"nessuno"}</div><button data-e="${q.id}">Modifica</button><button data-as="${q.id}">Assegna</button></div>`).join("")}`;else b.innerHTML=`<div class="card"><button id="export">Esporta backup JSON</button><input id="import" type="file" accept=".json"><p class="small">Il backup contiene profili, XP, Pokémon, Quest, assegnazioni e impostazioni.</p><hr><input id="pin" type="password" placeholder="Nuovo PIN"><button id="pinb">Cambia PIN</button></div>`;bindAdmin()}
function bindAdmin(){$("#add")?.addEventListener("click",()=>{let n=prompt("Nome?");if(!n)return;let a=Math.max(1,Math.min(14,+prompt("Età (1-14)?","7")||7)),k=kid(n,a);s.kids.push(k);s.assign[k.id]=[];ensure();save().then(()=>{render();adminPane("kids")})});document.querySelectorAll("[data-save]").forEach(b=>b.onclick=async()=>{let k=s.kids.find(x=>x.id===b.dataset.save);k.name=$(`[data-n="${k.id}"]`).value||k.name;k.age=Math.max(3,Math.min(14,+$(`[data-age="${k.id}"]`).value||7));s.assign[k.id]=[];ensure();await save();render();adminPane("kids")});document.querySelectorAll("[data-del]").forEach(b=>b.onclick=async()=>{if(!confirm("Eliminare il profilo?"))return;let i=b.dataset.del;s.kids=s.kids.filter(k=>k.id!==i);delete s.assign[i];active=s.kids[0]?.id;await save();render();adminPane("kids")});$("#newq")?.addEventListener("click",()=>{let t=prompt("Testo Quest");if(!t)return;s.lib.push({id:id(),icon:prompt("Icona","⭐")||"⭐",text:t});save().then(()=>adminPane("quests"))});document.querySelectorAll("[data-e]").forEach(b=>b.onclick=async()=>{let q=s.lib.find(x=>x.id===b.dataset.e),t=prompt("Testo",q.text);if(t===null)return;q.text=t;q.icon=prompt("Icona",q.icon)||q.icon;await save();adminPane("quests");render()});document.querySelectorAll("[data-as]").forEach(b=>b.onclick=()=>{let q=s.lib.find(x=>x.id===b.dataset.as),lines=s.kids.map((k,i)=>`${i+1}. ${k.name}`).join("\\n"),v=prompt(`Assegna "${q.text}" a quali bambini? Numeri separati da virgola.`,lines);if(v===null)return;v.split(",").forEach(x=>{let i=+x.trim()-1;if(i>=0&&i<s.kids.length){s.assign[s.kids[i].id]??=[];if(!s.assign[s.kids[i].id].includes(q.id))s.assign[s.kids[i].id].push(q.id)}});save().then(()=>{adminPane("quests");render()})});$("#export")?.addEventListener("click",()=>{let a=document.createElement("a");a.href=URL.createObjectURL(new Blob([JSON.stringify(s,null,2)],{type:"application/json"}));a.download="pokemon-quest-backup.json";a.click()});$("#import")?.addEventListener("change",async e=>{let f=e.target.files[0];if(!f)return;try{s=JSON.parse(await f.text());ensure();await save();active=s.kids[0]?.id;render();alert("Backup ripristinato")}catch{alert("Backup non valido")}});$("#pinb")?.addEventListener("click",async()=>{let p=$("#pin").value;if(p.length<4)return alert("PIN troppo corto");s.pin=p;await save();alert("PIN cambiato")})}
