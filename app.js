...
function minFiveXP(k){let a=variableCandidates(k).map(x=>x.xp).sort((x,y)=>x-y);return a.length>=5?a.slice(0,5).reduce((a,b)=>a+b,0):0}
function commonDailyTarget(){if(!s.kids.length)return 0;return Math.max(...s.kids.map(minFiveXP))}
function balancedFive(k,target){let vars=variableCandidates(k);if(vars.length<5)return [];vars.sort((a,b)=>a.xp-b.xp||a.q.text.localeCompare(b.q.text));let best=null;for(let i=0;i<vars.length;i++)for(let j=i+1;j<vars.length;j++)for(let h=j+1;h<vars.length;h++)for(let m=h+1;m<vars.length;m++)for(let n=m+1;n<vars.length;n++){let chosen=[vars[i],vars[j],vars[h],vars[m],vars[n]];let base=chosen.reduce((a,x)=>a+x.xp,0);if(base>target)continue;let cap=chosen.reduce((a,x)=>a+(20-x.xp),0);if(base+cap<target)continue;let need=target-base;let xs=chosen.map(x=>x.xp);for(let z=0;z<xs.length&&need>0;z++){let add=Math.min(20-xs[z],need);add-=add%5;xs[z]+=add;need-=add}if(need===0){let score=base; if(!best||score>best.base)best={chosen,xs,base};}}return best?best.chosen.map((x,i)=>({q:x.q,xp:best.xs[i]})):[]}
function dailyPlan(k){let cache=k.dailyPlan?.[today()];if(Array.isArray(cache)&&cache.length===5&&cache.every(x=>s.lib.some(q=>q.id===x.id))){let total=cache.reduce((a,x)=>a+Number(x.xp||0),0),target=commonDailyTarget();if(total===target)return cache.map(x=>({q:s.lib.find(q=>q.id===x.id),xp:Number(x.xp)}))}let target=commonDailyTarget(),result=balancedFive(k,target);if(result.length<5){let vars=variableCandidates(k).slice().sort((a,b)=>a.xp-b.xp||a.q.text.localeCompare(b.q.text));result=vars.slice(0,5).map(x=>({q:x.q,xp:x.xp}));}k.dailyPlan=k.dailyPlan||{};k.dailyPlan[today()]=result.map(x=>({id:x.q.id,xp:x.xp}));return result}
...
function init(){s={version:0,pin:"1234",kids:[kid("Giuseppe",9,"rosso"),kid("Anna Chiara",7,"rosa"),kid("Elisabetta",5,"viola"),kid("Miriam",2,"verde")],lib:[],assign:{},groupLib:GROUPS.map(([icon,text,xp])=>({id:id(),icon,text,xp})),group:{date:today(),qid:null,done:{}},groupHistory:{},reward:{small:350,big:850}};seedApprovedPools();ensure()}
function seedApprovedPools(){const texts=[...new Set(P.flatMap(x=>x[2].map(y=>y[1])))];for(const [icon,text] of P.flatMap(x=>x[2])){if(!s.lib.some(q=>q.text===text))s.lib.push({id:id(),icon,text,xp:xpForText(text)})}for(const k of s.kids){s.assign[k.id]=s.assign[k.id]||[];for(const text of texts){let q=s.lib.find(x=>x.text===text);if(q&&!s.assign[k.id].includes(q.id))s.assign[k.id].push(q.id)}}}
function ensure(){
...
function bindAdmin(){
 $("#add")?.addEventListener("click",()=>{let n=prompt("Nome?");if(!n)return;let a=Math.max(1,Math.min(14,+prompt("Età (1-14)?","7")||7)),k=kid(n,a,"viola");k._new=true;s.kids.push(k);s.assign[k.id]=[];ensure();save().then(()=>{render();adminPane("kids")})});
