...
function kid(n,a,theme="viola",avatar="none"){return{id:id(),name:n,age:a,xp:0,weekly:0,line:0,theme:THEME_KEYS.includes(theme)?theme:"viola",avatar:avatar||"none",done:{}}}
...
function render(){
 ...
 let tabs=s.kids.map(x=>`<button class="${x.id===active?"active":""}" data-k="${x.id}">${x.avatar && x.avatar!=="none" ? esc(x.avatar)+" " : ""}${esc(x.name)}</button>`).join("");
