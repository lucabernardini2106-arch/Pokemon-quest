...
function kid(n,a,theme="viola"){return{id:id(),name:n,age:a,xp:0,weekly:0,line:0,theme:THEME_KEYS.includes(theme)?theme:"viola",done:{}}}
...
function ensure(){
 ...
 s.kids.forEach(k=>{k.age=Math.max(1,Math.min(14,Number(k.age)||7));k.theme=THEME_KEYS.includes(k.theme)?k.theme:"viola";k.xp=Number(k.xp)||0;k.weekly=Number(k.weekly)||0;k.line=Number(k.line)||0;k.done=k.done&&typeof k.done==="object"?k.done:{};s.assign[k.id]=Array.isArray(s.assign[k.id])?s.assign[k.id]:[];pool(Math.max(3,k.age)).forEach(([ic,t])=>{let q=s.lib.find(x=>x.text===t);if(!q){q={id:id(),icon:ic,text:t};s.lib.push(q)}if(!s.assign[k.id].includes(q.id))s.assign[k.id].push(q.id)})});
