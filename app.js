...
function xpForText(t){let x=String(t).toLowerCase();if(/20 minuti|20 min|studio|attività fisica|caccia al tesoro|rompicapo|puzzle/.test(x))return 20;if(/15 minuti|15 min|senza distrazioni|autonomamente|senza che te lo chied/.test(x))return 15;if(/10 minuti|10 min|leggi|leggere|libro|impara|imparato|organizza|responsabilità/.test(x))return 10;return 5}
function ageXP(k,q){let a=k.age,x=String(q.text).toLowerCase();if(a<=2){if(/aiuto/.test(x))return 15;if(/scarpe|vestito/.test(x))return 10;return 5;}if(a<=4){if(/riordinare una stanza/.test(x))return 15;if(/aiuto/.test(x))return 15;return 5;}if(a<=6){if(/aiuta un fratello|aiuta in casa|riordina tutti i giochi|riordina ciò che hai usato|pianta|responsabilità/.test(x))return 15;return 5}if(a<=8){if(/leggi|fratello|responsabilità|riordina la tua stanza|riordina ciò che hai usato|gentile/.test(x))return 10;return 5}if(a<=10){if(/leggi|fratello|responsabilità|imparato|camera|aiuta qualcuno|riordina ciò che hai usato/.test(x))return 10;return 5}return /20 minuti|attività|responsabilità|migliorare|imparato|impegni/.test(x)?10:5}
function seedFor(k){return [...today(),...String(k.id)].reduce((a,c)=>((a*31+c.charCodeAt(0))>>>0),7)}
...
function commonDailyTarget(){let sets=s.kids.map(possibleSums).filter(x=>x.size);if(!sets.length)return 25;let common=[...sets[0]].filter(v=>sets.every(x=>x.has(v)));if(!common.length){let all=[...new Set(sets.flatMap(x=>[...x]))];common=all.sort((a,b)=>{let da=sets.reduce((z,x)=>z+(x.has(a)?0:1),0),db=sets.reduce((z,x)=>z+(x.has(b)?0:1),0);return da-db||Math.abs(a-50)-Math.abs(b-50)})}if(!common.length)return 25;let seed=[...today()].reduce((a,c)=>((a*31+c.charCodeAt(0))>>>0),17);return common[seed%common.length]??common[0]}
function dailyPlan(k){let cache=k.dailyPlan?.[today()];if(Array.isArray(cache)&&cache.length)return cache.map(x=>({q:s.lib.find(q=>q.id===x.id),xp:x.xp})).filter(x=>x.q);let vars=variableCandidates(k),target=commonDailyTarget(),combo=combinations(vars.map(x=>({id:x.q.id,xp:x.xp})),5,target),result=combo.length?combo[0].map(x=>({q:s.lib.find(q=>q.id===x.id),xp:x.xp})):[];if(result.length<5){result=vars.slice(0,5)}k.dailyPlan=k.dailyPlan||{};k.dailyPlan[today()]=result.map(x=>({id:x.q.id,xp:x.xp}));return result}
function dailyQs(k){let fixed=FIXED.map(f=>{let q=s.lib.find(x=>x.text===f.text);if(!q){q={id:id(),...f};s.lib.push(q)}return q});return fixed.concat(dailyPlan(k).map(x=>x.q));}
...
function pane(k){
 if(tab==="quests"){
  ...
  if(k.age<=6){
   ...
   ${qs.map(q=>{let d=done(k,q);return `<div class="quest ${d?"done":""}"><span class="icon">${q.icon}</span><span class="text">${esc(q.text)} <span class="small">+${dailyXP(k,q)} XP</span></span><button data-speak="${esc(q.text)}">🔊</button><button data-q="${q.id}">${d?"↩️":"✅"}</button></div>`}).join("")}
  ...
  ${qs.map(q=>{let d=done(k,q);return `<div class="quest ${d?"done":""}"><span class="icon">${q.icon}</span><span class="text">${esc(q.text)} <span class="small">+${dailyXP(k,q)} XP</span></span><button data-q="${q.id}">${d?"↩️ Annulla":"✅ Fatto"}</button></div>`}).join("")}
...
function bindAdmin(){
 $("#add")?.addEventListener("click",()=>{let n=prompt("Nome?");if(!n)return;let a=Math.max(1,Math.min(14,+prompt("Età (1-14)?","7")||7)),k=kid(n,a,"viola");s.kids.push(k);s.assign[k.id]=[];FIXED.forEach(f=>{let q=s.lib.find(x=>x.text===f.text);if(!q){q={id:id(),...f};s.lib.push(q)}s.assign[k.id].push(q.id)});P.find(x=>a>=x[0]&&a<=x[1])[2].forEach(([ic,t])=>{let q=s.lib.find(x=>x.text===t);if(!q){q={id:id(),icon:ic,text:t,xp:xpForText(t)};s.lib.push(q)}if(!s.assign[k.id].includes(q.id))s.assign[k.id].push(q.id)});ensure();save().then(()=>{render();adminPane("kids")})});
 document.querySelectorAll("[data-save]").forEach(b=>b.onclick=async()=>{let k=s.kids.find(x=>x.id===b.dataset.save);k.name=$(`[data-n="${k.id}"]`).value||k.name;let oldAge=k.age;let newAge=Math.max(1,Math.min(14,+$(`[data-age="${k.id}"]`).value||7));k.age=newAge;if(oldAge!==newAge)k.dailyPlan={};k.theme=$(`[data-theme="${k.id}"]`).value||"viola";k.avatar=$(`[data-avatar="${k.id}"]`).value||"none";s.assign[k.id]=Array.isArray(s.assign[k.id])?s.assign[k.id]:[];ensure();await save();render();adminPane("kids")});
